...
def generate_simout(jobid = None, resultsdir = None, partial = None, output = sys.stdout, silent = False):
  ...
  for tlb in ('itlb', 'dtlb', 'stlb'):
	...
	('    num allocs', '%s.allocs'%tlb, str),
	('    num bypass', '%s.bypass'%tlb, str),
        ('    miss rate', '%s.missrate'%tlb, lambda v: '%.2f%%' % v),
        ('    mpki', '%s.mpki'%tlb, lambda v: '%.2f' % v),
        ('    num changes' , '%s.change'%tlb, str),
      ])
  ...
  for c in existcaches:
    ...
    template.extend([
      ...
      ('    predictor llc accesses', '%s.llcAcc'%c , str),
      ('    predictor llc bypass count', '%s.llcBypass'%c , str),
      ('    predictor llc bypass misses', '%s.llcMiss'%c , str),
      ('    predictor llc evictions', '%s.llcEvictions'%c , str),
      ('    predictor llc default misses', '%s.llcMissDef'%c , str),
    ])
