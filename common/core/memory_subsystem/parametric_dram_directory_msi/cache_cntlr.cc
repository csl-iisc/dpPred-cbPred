   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   registerStatsMetric(name, core_id, "llcAcc", &llcAcc);
   registerStatsMetric(name, core_id, "llcBypass", &llcBypass);
   registerStatsMetric(name, core_id, "llcMiss", &llcMiss);
   registerStatsMetric(name, core_id, "llcEvictions", &llcEvictions);
   registerStatsMetric(name, core_id, "llcMissDef", &llcMissDef);
...
CacheCntlr::setDRAMDirectAccess(DramCntlrInterface* dram_cntlr, UInt64 num_outstanding)
{
   ...
   //std::cout << "Inside setDramDirectAccess" << std::endl;
...
LOG_ASSERT_ERROR(offset + data_length <= getCacheBlockSize(), "access until %u > %u", offset + data_length, getCacheBlockSize());
   ...
   if (!cache_hit && m_perfect)
   {
      ...
      {
         //std::cout << "1st call point " << std::endl;
...
MYLOG("processMemOpFromCore l%d before next", m_mem_component);
      hit_where = m_next_cache_cntlr->processShmemReqFromPrevCache(this, mem_op_type, ca_address, modeled, count, Prefetch::NONE, t_start, false, true);
...
//std::cout << m_mem_component << std::endl;
...
MYLOG("processMemOpFromCore l%d before next fill", m_mem_component);
         hit_where = m_next_cache_cntlr->processShmemReqFromPrevCache(this, mem_op_type, ca_address, false, false, Prefetch::NONE, t_start, true, false);

//std::cout << hit_where << std::endl;
...
MYLOG("processMemOpFromCore l%d after next fill", m_mem_component);
      ...
      cache_block_info = getCacheBlockInfo(ca_address);
     // std::cout << isBypassed << "      " << m_mem_component << std::endl;
      //if (m_mem_component == 3 && isBypassed)
     // {
          // std::cout << "LLC miss" << std::endl;
       //     ((SharedCacheBlockInfo*)cache_block_info)->setDead(1);
        // std::cout << ((SharedCacheBlockInfo*)cache_block_info)->getDead() << std::endl;
     // }
      #ifdef PRIVATE_L2_OPTIMIZATION
      #else
      if (!lock_all)
...
MYLOG("copyDataFromNextLevel l%d", m_mem_component);
   ...
   else
   {
      ...
      //  std::cout << "2nd call point " << std::endl;
...
HitWhere::where_t
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock, bool onCacheMiss)
{
   #ifdef PRIVATE_L2_OPTIMIZATION
   ...
   else // !cache_hit: either data is not here, or operation on data is not permitted
   {
      ...
      if (m_next_cache_cntlr)
      {
         ...
         hit_where = m_next_cache_cntlr->processShmemReqFromPrevCache(this, mem_op_type, address, modeled, count, isPrefetch == Prefetch::NONE ? Prefetch::NONE : Prefetch::OTHER, t_issue, have_write_lock_internal, onCacheMiss);
         if (hit_where != HitWhere::MISS)
         {
            cache_hit = true;
...
bool
CacheCntlr::accessCacheLine(IntPtr addr)
{
   return m_master->m_cache->peekSingleLine(addr) != NULL;
}
...
SharedCacheBlockInfo*
CacheCntlr::insertCacheBlock(IntPtr address, CacheState::cstate_t cstate, Byte* data_buf, core_id_t requester, ShmemPerfModel::Thread_t thread_num, bool isLLC)
{
MYLOG("insertCacheBlock l%d @ %lx as %c (now %c)", m_mem_component, address, CStateString(cstate), CStateString(getCacheState(address)));
   ...
   m_master->m_cache->insertSingleLine(address, data_buf,
         ...
         getShmemPerfModel()->getElapsedTime(thread_num), this, isLLC);
...
MYLOG("processExRepFromDramDirectory l%d", m_mem_component);
  ...
  // std::cout << "5th call point " << std::endl;
//insertCacheBlock(address, CacheState::EXCLUSIVE, data_buf, requester, ShmemPerfModel::_SIM_THREAD);
insertCacheBlock(address, CacheState::EXCLUSIVE, data_buf, requester, ShmemPerfModel::_SIM_THREAD, true);
...
//std::cout << "6th call point " << std::endl;  // Insert Cache Block in L2 Cache
