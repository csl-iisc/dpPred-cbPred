   ...
   m_enabled(false)
{
   ...
   try
   {
      ...
      if (stlb_size)
         m_stlb = new TLB("stlb", "perf_model/stlb", getCore()->getId(), stlb_size, Sim()->getCfg()->getInt("perf_model/stlb/associativity"), NULL, Sim()->getCfg()->getInt("perf_model/stlb/conf"));
   ...
   if (m_core_id_master == getCore()->getId())
   {
     ...
     m_dtlb->setL3Controller(m_cache_cntlrs[m_last_level_cache]);
     m_stlb->setL3Controller(m_cache_cntlrs[m_last_level_cache]);
     m_itlb->setL3Controller(m_cache_cntlrs[m_last_level_cache]);
   }
...
void 
MemoryManager::setDeadHelper (TLB * tlb, IntPtr address){
    tlb->setDeadBit (address);
}    
...
{
   bool hit = tlb->lookup(address, getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD), isIfetch, this);
   if (hit == false
