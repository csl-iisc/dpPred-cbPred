...
void
CacheSet::insert(CacheBlockInfo* cache_block_info, Byte* fill_buff, bool* eviction, CacheBlockInfo* evict_block_info, Byte* evict_buff, CacheCntlr *cntlr, bool LLC, bool L2TLB)
{
   ...
   const UInt32 index = getReplacementIndex(cntlr, LLC, L2TLB);
   ...
   if (m_cache_block_info_array[index]->isValid())
   {
      ...
      *eviction = true;
      ...
      if (dynamic_cast<SharedCacheBlockInfo*>(m_cache_block_info_array[index]) != nullptr) {
	SharedCacheBlockInfo* c = dynamic_cast<SharedCacheBlockInfo*>(m_cache_block_info_array[index]);
	((SharedCacheBlockInfo*)evict_block_info)->cloneAIP(c);
      }
   if (dynamic_cast<PrL1CacheBlockInfo*>(m_cache_block_info_array[index]) != nullptr) {
        PrL1CacheBlockInfo* c = dynamic_cast<PrL1CacheBlockInfo*>(m_cache_block_info_array[index]);
        ((PrL1CacheBlockInfo*)evict_block_info)->cloneAIPTLB(c);
   ...
   if (dynamic_cast<PrL1CacheBlockInfo*>(m_cache_block_info_array[index]) != nullptr) {
	SharedCacheBlockInfo* c = dynamic_cast<SharedCacheBlockInfo*>(m_cache_block_info_array[index]);
        ((SharedCacheBlockInfo*)evict_block_info)->setDead(0);
      }
   if (dynamic_cast<PrL1CacheBlockInfo*>(m_cache_block_info_array[index]) != nullptr) {
        PrL1CacheBlockInfo* c = dynamic_cast<PrL1CacheBlockInfo*>(m_cache_block_info_array[index]);
        ((PrL1CacheBlockInfo*)evict_block_info)->setDead(0);

      }
 
   //m_cache_block_info_array[index]->setSet(this);
