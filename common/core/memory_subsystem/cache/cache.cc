...
void 
Cache::increaseCForEntireSet (IntPtr addr)
{
  ...
  IntPtr tag;
   ...
   splitAddress(addr, tag, set_index);
   assert(set_index < m_num_sets);
   ((CacheSetLRU*) m_sets[set_index]) -> incrementSetCount();
...
void
Cache::modifyCounterForEntireSet (IntPtr addr, std::map<IntPtr, std::pair<uint64_t, IntPtr>> &phy_table)
{
   ...
   splitAddress(addr, tag, set_index);
   assert(set_index < m_num_sets);
   ((CacheSetLRU*) m_sets[set_index]) -> modifySetCounters(addr, phy_table);
...
void 
Cache::setPrDeadBit (IntPtr address){
   IntPtr tag;
   UInt32 set_index;
   UInt32 line_index = -1;
   UInt32 block_offset;

   splitAddress(address, tag, set_index, block_offset);

   CacheSet* set = m_sets[set_index];
   CacheBlockInfo* cache_block_info = set->find(tag, &line_index);

   if (dynamic_cast<PrL1CacheBlockInfo*>(cache_block_info) != nullptr)
   {
      ((PrL1CacheBlockInfo*)cache_block_info)->setDead(1);
   }
...
Cache::insertSingleLine(IntPtr addr, Byte* fill_buff,
      ...
      SubsecondTime now, CacheCntlr *cntlr, bool LLC, bool L2TLB)
{
   IntPtr tag;
   UInt32 set_index;
   splitAddress(addr, tag, set_index);
   ...
   //cache_block_info->setSet(set_index);

   m_sets[set_index]->insert(cache_block_info, fill_buff,
         eviction, evict_block_info, evict_buff, cntlr, LLC, L2TLB);
